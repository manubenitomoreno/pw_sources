...
pois AS (
  SELECT 
    id, 
    COALESCE(NULLIF(cast(data ->> 'Origins - Housing - Single Family Residence - Number' AS FLOAT) +
                    cast(data ->> 'Origins - Housing - Collective Housing - Number' AS FLOAT),'NaN'), 0) AS housing_number,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Healthcare - Multiple - Number' AS FLOAT) +
                    cast(data ->> 'Destinations - Healthcare - Public - Number' AS FLOAT),'NaN'), 0) AS care_number,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Educational - Superior - Number' AS FLOAT) +
                    cast(data ->> 'Destinations - Educational - Basic - Number' AS FLOAT),'NaN'), 0) AS school_number,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Leisure - Bars and Restaurants - Number' AS FLOAT) +
                    cast(data ->> 'Destinations - Leisure - Cultural - Number' AS FLOAT) +
                    cast(data ->> 'Destinations - Leisure - Shows - Number' AS FLOAT),'NaN'), 0) AS leisure_number,
	COALESCE(NULLIF(cast(data ->> 'Destinations - Retail - Mall - Number' AS FLOAT) +
					cast(data ->> 'Destinations - Retail - Market or Supermarket - Number' AS FLOAT) +
					cast(data ->> 'Destinations - Retail - Stand Alone Retail - Number' AS FLOAT),'Nan'), 0) AS shopping_number,
	COALESCE(NULLIF(cast(data ->> 'Destinations - Sport - Multiple - Number' AS FLOAT),'Nan'), 0) AS sport_number,
    COALESCE(NULLIF(cast(data ->> 'Origins - Housing - Single Family Residence - Area' AS FLOAT) +
                    cast(data ->> 'Origins - Housing - Collective Housing - Area' AS FLOAT),'NaN'), 0) AS housing_area,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Healthcare - Multiple - Area' AS FLOAT) +
                    cast(data ->> 'Destinations - Healthcare - Public - Area' AS FLOAT),'NaN'), 0) AS care_area,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Educational - Superior - Area' AS FLOAT) +
                    cast(data ->> 'Destinations - Educational - Basic - Area' AS FLOAT),'NaN'), 0) AS school_area,
    COALESCE(NULLIF(cast(data ->> 'Destinations - Leisure - Bars and Restaurants - Area' AS FLOAT) +
                    cast(data ->> 'Destinations - Leisure - Cultural - Area' AS FLOAT) +
                    cast(data ->> 'Destinations - Leisure - Shows - Area' AS FLOAT),'NaN'), 0) AS leisure_area,
	COALESCE(NULLIF(cast(data ->> 'Destinations - Retail - Mall - Area' AS FLOAT) +
					cast(data ->> 'Destinations - Retail - Market or Supermarket - Area' AS FLOAT) +
					cast(data ->> 'Destinations - Retail - Stand Alone Retail - Area' AS FLOAT),'Nan'), 0) AS shopping_area,
	COALESCE(NULLIF(cast(data ->> 'Destinations - Sport - Multiple - Area' AS FLOAT),'Nan'), 0) AS sport_area,
	COALESCE(NULLIF(cast(data ->> 'Other - Office - Area' as FLOAT) +
					cast(data ->> 'Other - Industrial - Area' as FLOAT),'Nan'), 0) as work_area,
	...
	COALESCE(NULLIF(cast(data ->> 'Building Year Built' as float),'Nan'), 0) as year_built,
...
loaded_pois AS (
  SELECT
	...
	pois.storage_area,
	pois.parking_area,
	(pois.parcel_built_ag - (pois.housing_area + pois.care_area + pois.school_area + pois.leisure_area + pois.shopping_area + pois.sport_area + pois.work_area + pois.storage_area + pois.parking_area)) as other_area,
