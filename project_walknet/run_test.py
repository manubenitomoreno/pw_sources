...
def list_available_executions():
    """List available executions based on executions.yaml."""
    try:
        import yaml
        with open("executions.yaml", "r") as yaml_file:
            executions_metadata = yaml.safe_load(yaml_file)
        logger.info("Available executions:")
        for execution_name in executions_metadata:
            logger.info(f"- {execution_name}")
    except FileNotFoundError:
...
def main():
    ...
    group.add_argument("-e", "--execution", action="store_true", help="Indicate that the operation is for an execution.")

    parser.add_argument("keyname", nargs="?", help="The keyname of the source, network, or execution.")
    ...
    parser.add_argument("--list-executions", action="store_true", help="List available executions and exit.")
    ...
    # Handle Execution operations
        ...
        if args.keyname:
            ...
            start_time = time.time()
            pipeline_logger.bind(pipeline="execution").info(f"{args.keyname} - Execution started at: {time.strftime('%Y-%m-%d %H:%M:%S')}")
            execution.run()
            execution_time = time.time() - start_time
            pipeline_logger.bind(pipeline="execution").info(f"{args.keyname} - Execution finished in {execution_time:.2f} seconds.")
        else:
            print("Please provide a keyname for the execution.")

    else:
        parser.error("Please specify --source, --network, or --execution with a keyname and action, or use --list-sources, --list-executions, --config-params, or --reset-* options.")
